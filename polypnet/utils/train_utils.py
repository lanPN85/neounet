...
def log_exp_hyperparams(_logger, config):
    ...
    with tempfile.NamedTemporaryFile("w+t", suffix=".yml") as f:
        ...
        _logger.experiment.log_artifact(_logger.run_id, f.name)

    model_params = dict([(f"model.{k}", v) for (k, v) in config.model.kwargs.items()])
    data_params = dict([(f"data.{k}", v) for (k, v) in config.dataset.kwargs.items()])
    ...
    optimizer_params = dict(
        [(f"optimizer.{k}", v) for (k, v) in config.optimizer.kwargs.items()]
    )
    scheduler_params = dict(
        [(f"scheduler.{k}", v) for (k, v) in config.scheduler.kwargs.items()]
    )
    augment_params = dict(
        [(f"augment.{k}", v) for (k, v) in config.augment.kwargs.items()]
    )

    agc_params = dict([(f"agc.{k}", v) for (k, v) in config.agc.kwargs.items()])

    # Add loss parameters
    loss_params = dict([(f"loss.{k}", v) for (k, v) in config.loss.kwargs.items()])
    for i, c in enumerate(config.loss.losses):
        loss_params[f"loss.{i + 1}.class"] = c.cls
        loss_params.update(
            dict([(f"loss.{i + 1}.{k}", v) for (k, v) in c.kwargs.items()])
        )

    label_loss_params = dict(
        [(f"label_loss.{k}", v) for (k, v) in config.label_loss.kwargs.items()]
    )
    for i, c in enumerate(config.label_loss.losses):
        label_loss_params[f"label_loss.{i + 1}.class"] = c.cls
        label_loss_params.update(
            dict([(f"label_loss.{i + 1}.{k}", v) for (k, v) in c.kwargs.items()])
        )

    wrapper_params = dict(
        [(f"wrapper.{k}", v) for (k, v) in config.wrapper.kwargs.items()]
    )
