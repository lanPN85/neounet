...
def mask_collate_fn(batch):
    image_t = torch.stack([b[0] for b in batch])
    mask_t = torch.stack([b[1] for b in batch], dim=0)  # B x 3 x H x W
...
def triplet_collate_fn(batch):
    image_t = torch.stack([b[0] for b in batch])
    mask_t = torch.stack([b[1] for b in batch], dim=0)  # B x 3 x H x W
    cls_t = torch.stack([b[2] for b in batch], dim=0)  # B x 3 x H x W
...
def triplet_collate_fn_2(batch):
    ...
    if len(batch[0]) > 3:
        ids = [b[3].split("/")[-1].split(".")[0] for b in batch]
        out.append(ids)
