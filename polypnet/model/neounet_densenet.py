...
class NeoUNetDenseNet(nn.Module):
    ...
    def encoder_forward(self, x):
        outputs = []
        out = x
        for name, layer in self.encoder.features.named_children():
            out = layer(out)
            if name.startswith("transition") or name == 'norm5':
                outputs.append(out)
        return outputs
    ...
    def forward(self, x):
        ...
        o0, o1, o2, o3 = self.encoder_forward(x)

        # Bottom decoder
        attn_mid = self.attn_mid(o2, o3)
        up_mid = o3

        merged_2 = torch.cat((up_mid, attn_mid), dim=1)
...
def test_1():
    ...
    net = NeoUNetDenseNet(num_classes=2)
    outputs = net(x)
