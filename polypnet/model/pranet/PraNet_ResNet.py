...
class aggregation(nn.Module):
    ...
    # used after MSF
    def __init__(self, channel, num_classes=1):
...
class CRANet(nn.Module):
    ...
    def set_num_classes(self, num_classes: int):
        ...
        self.agg1 = aggregation(self.channel, num_classes)
        self.ra3_conv4 = BasicConv2d(64, num_classes, kernel_size=3, padding=1)
        self.ra2_conv4 = BasicConv2d(64, num_classes, kernel_size=3, padding=1)
        self.ra4_conv5 = BasicConv2d(256, num_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x = x[:, -1, ...].unsqueeze(1).expand(-1, 2048, -1, -1).mul(x4)
        x = self.ra4_conv1(x)
        ...
        x = x[:, -1, ...].unsqueeze(1).expand(-1, 1024, -1, -1).mul(x3)
        x = self.ra3_conv1(x)
        ...
        x = x[:, -1, ...].unsqueeze(1).expand(-1, 512, -1, -1).mul(x2)
        x = self.ra2_conv1(x)
