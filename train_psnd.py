...
from polypnet.callbacks import MLFlowModelCheckpoint, ResultSampleCallback, CsvLogCallback
...
def main():
    ...
    for c in config.loss.losses:
        ...
        losses.append(l)
    seg_loss = MultiscaleLoss(ConditionalCompoundLoss(
    ...
    for c in config.label_loss.losses:
        l = load_class_from_conf(c)
        losses.append(l)
    cls_loss = MultiscaleLoss(ConditionalCompoundLoss(
        losses, **config.label_loss.kwargs
    ))
    ...
    if config.from_segment_model is not None:
        logger.info(f"Using weights from segmentation model at '{config.from_segment_model}'")
        model.set_num_classes(1)
        seg_wrapper = UnetModelWrapper.load_from_checkpoint(
            config.from_segment_model, strict=False,
    ...
    if config.with_mlflow:
        ...
        callbacks.append(MLFlowModelCheckpoint(
            _logger,
            **common_ckpoint_args
        ))
    else:
    ...
    callbacks.append(CsvLogCallback(
        os.path.join(result_dir, "per-image.csv"),
